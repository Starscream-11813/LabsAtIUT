...
//    if(x->right!=NULL)
//    {
//        return minvaluennode(x->right);
//    }
//    Node* p=x->parent;
//    while()
//}
void foo(Node* root, Node** pred, Node** succ, int key)
{
    if(root==NULL)
    {
        return;
    }
    foo(root->left,pred,succ,key);
    if(root!=NULL && root->data>key)
    {
        if(*succ==NULL || (*succ!=NULL && (*succ)->data>root->data))
        {
            *succ=root;///successor (node er porer value tah)
        }
    }
    else if(root!=NULL && root->data<key)
    {
        *pred=root;///predecessor (node er ager value)
    }
    foo(root->right,pred,succ,key);
}
...
int main()
{
    ...
    while(q--)
    {
        ...
        if(searchkey(root,x))
        {
            foo(root,&pred,&succ,x);
            if(pred==NULL)
            {
                cout<<"NULL"<<" "<<succ->data<<endl;
            }
            else if(succ==NULL)
            {
                cout<<pred->data<<" "<<"NULL"<<endl;
            }
            else
            {
