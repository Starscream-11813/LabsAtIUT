...
int maxbetweennodeandancestor(Node* node, int key)
{
    int maxm=-999999999;
    Node* temp=node;
    while(temp->data!=key)
...
int maxbetweentwonodes(Node* node, int a, int b)
{
    Node* temp=node;
    while((a<temp->data && b<temp->data) || (a>temp->data && b>temp->data))
    {
    ...
    return max(maxbetweennodeandancestor(temp,a),maxbetweennodeandancestor(temp,b));
}
int main()
{
    //Godspeed;
    ...
    while(q--)
    {
        int x,y;
        cin>>x>>y;
        cout<<maxbetweentwonodes(root,x,y)<<endl;
    }
    Bye;
}
