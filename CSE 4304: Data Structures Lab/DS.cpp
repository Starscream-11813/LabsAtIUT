...
int main()
{
    ...
    while(t--)
    {
        ...
        a.pb(make_tuple(dd,mm,yy));
        int bd,bm,by;
        scanf("%d.%d.%d",&bd,&bm,&by);
        a.pb(make_tuple(bd,bm,by));
        bool flag=false;
        int swaps=0;
top:
        if(get<2>(a[0])-get<2>(a[1])>=18)
        {
            if(get<1>(a[0])>=get<1>(a[1]))
            {
