...
int main()
{
    ...
    while(Tests--)
    {
        ...
        REP(i,n)
        {
            cin>>a[i];
            m[a[i]]++;
            dp[a[i]]=m[a[i]];///I need to maximize the sum of numbers taken.
        }
        ll maxm=*max_element(all(a));
        FOR(i,2,maxm)
        {
            dp[i]=max(dp[i-1],dp[i-2]+(i*dp[i]));///either i-1 ke nibo or i-2 ke nibo along with i*dp[i].
        }
        cout<<dp[maxm];///Bottom-up Soln.
    }
    Bye;
}
