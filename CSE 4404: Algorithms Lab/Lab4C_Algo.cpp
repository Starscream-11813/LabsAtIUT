...
{
    LD res=atan2(y,x);
    return (res >= 0 ? res : (2.0*PI + res)) * 360.0 / (2.0*PI);
}
...
int main()
{
    ...
    while(Tests--)
    {
        ll n;
        cin>>n;
        //vector<pair<LD,pair<LD,LD> > >p;///<theta,<x,y>>.
        deque<pair<LD,pair<LD,LD> > >p;
        vector<pair<LD,LD> >c;
        while(n--)
        {
            LD x,y;
        ...
        REP(i,p.size())
        {
            if((ll)p[0].ss.ff!=(ll)c[0].ff && (ll)p[0].ss.ss!=(ll)c[0].ss)
            {
                p.pb(p.front());
                p.pop_front();
            }
        ...
        REP(i,p.size())
        {
            cout<<(ll)p[i].ss.ff<<" "<<(ll)p[i].ss.ss<<endl;
        }
    }
    Bye;
}
