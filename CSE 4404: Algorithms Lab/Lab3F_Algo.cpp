...
#define endl                    '\n'
const long long int MOD = 998244353;//1000000007; // 998244353
const ll INF = 1e18;
...
*/
ll n,k;
ll dp[1001][1001];///states: minimum difficulty and no. of problems.
                ///dp[i][j] stores the number of possible selection of j problems with mindiff i.
                ///either pick a mindiff difficulty level problem in m[mindiff] ways, and other probs-1 problems from difficulty mindiff+1.
                ///or, discard all problems of mindiff difficulty level, and pick all the problems from difficulty mindiff+1.
unordered_map<ll,ll>m;
ll foo(ll probs, ll mindiff)
{
    if(probs>m.size()-mindiff)///No way to select probs different problems, when what remains after subtracting mindiff problems from total unique problems
                                ///is less than probs itself.
    {
        return 0;
    }
    else if(!probs)
    {
        return 1;///One way to select no problems at all.
    }
    else if(dp[mindiff][probs]!=-1)
    {
        return dp[mindiff][probs];
    }
    else
    {
        dp[mindiff][probs]=((((m[mindiff]*foo(probs-1,mindiff+1))%MOD)+foo(probs,mindiff+1))%MOD);
        return dp[mindiff][probs];
    }
}
int main()
