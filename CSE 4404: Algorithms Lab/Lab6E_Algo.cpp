...
ll adj[101][101],d[101][101];///adj[i][j]=edge weight between i and j.
                             ///d[i][j]=shortest path weight between i and j.
...
ll nP2(ll n)///nPr = n!/(n-r)! = n(n-1)(n-2)!/(n-2)! = n(n-1) = n^2 - n.
            ///(No. of possible pairs in n nodes)
{
    return (SQR(n)-n);
}
int main()
...
//        }
        ...
        //what_is(res);
        LD avg=((LD)res)/((LD)nP2((ll)s.size()));///Sum of all pair shortest paths/nP2.
        cout<<"Case "<<Tests<<": average length between pages = "<<fixed<<setprecision(3)<<avg<<" clicks"<<endl;
    }
    Bye;
}
