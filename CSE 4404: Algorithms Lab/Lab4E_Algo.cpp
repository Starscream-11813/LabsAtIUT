...
int main()
{
    ...
    while(Tests--)
    {
        ...
        while(true)
        {
            ...
            v.pb({f,s});///Ulta nisi karon sort toh korte hobe finishing time onujayee.
        }
        sort(all(v));
        ll lastF=v[0].ff;
        ll res=1;
        FOR(i,1,v.size()-1)
        {
            if(v[i].ss>=lastF)///Porer tar starting time jodi current tar finishing time er pore hoy, tahole compatible.
            {
                res++;
                lastF=v[i].ff;
            }
